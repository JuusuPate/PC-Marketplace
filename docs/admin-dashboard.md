...
`/admin/settings` sallii ylläpitäjän keskeyttää ja jatkaa uusien ilmoitusten luontia. Tietokannan `20260925171613_listing_creation_setting.sql`-migraatio tallentaa oletusarvoksi **sallittu**. Suojattu luku- ja tallennusfunktio tarkistaa ylläpitäjän palvelinpuolella joka kutsulla; tallennus käyttää versiota, jotta kaksi ylläpitäjää eivät kirjoita toistensa muutoksen päälle. Muutos kirjataan tapahtumalokiin.
...
Lisämigraatio `20260926082532_admin_user_and_model_trends.sql` laajentaa aiemman kaaviorajapinnan vastetta ja lisää suojatun `get_admin_model_price_trends`-lukurajapinnan. Asenna sekä `20260925190901_admin_dashboard_trends.sql` että tämä lisämigraatio ennen käyttöliittymän käyttöönottoa; **kumpaakaan ei ole tämän työn yhteydessä asennettu etäprojektiin**. Molemmat tarkistetaan todellisella migraatioketjulla PGlitessä. Oikean Supabasen vahvistus on erillinen käyttöönottovaihe.
